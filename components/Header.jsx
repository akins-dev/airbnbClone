...
function Header({ placeholder }) {
  ...
  // Url masking is also allowed. Hover on push for more info.
  const search =  () => {
    router.push({
      pathname: "/search",
      query: {
  ...
  return (
    <header className="sticky top-0 z-50 grid grid-cols-3 bg-white shadow-md p-3 md:px-8">
      {/* Left */}
      <div onClick={() => router.push("/")} className="relative flex items-center h-8 cursor-pointer disable-tap-highlight my-auto">
        <Image
